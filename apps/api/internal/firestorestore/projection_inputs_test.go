...
func TestPolicyRevisionImmediatelyHidesAndRejectsStaleRebuild(t *testing.T) {
	...
	if err := b.Policy().Upsert(ctx, p); err != nil {
		t.Fatal(err)
	}
	if err := b.Projection().Replace(ctx, "alice", now, now.Add(24*time.Hour), publicationFixtures(now, "old", 3)); err != nil {
		t.Fatal(err)
	}
	...
	if err := b.Projection().Replace(stale, "alice", now, now.Add(24*time.Hour), publicationFixtures(now, "stale", 3)); !errors.Is(err, errProjectionInputsChanged) {
		t.Fatalf("stale policy published: %v", err)
	}
	requirePublicationHidden(t, b, ctx)
	...
	if err := b.Projection().Replace(ctx, "alice", now.Add(time.Hour), now.Add(2*time.Hour), fresh); err != nil {
		t.Fatal(err)
	}
	got, err := b.Projection().ListForUser(ctx, "alice")
	if err != nil || len(got) != 1 || got[0].ID != fresh[0].ID {
		t.Fatalf("old policy rows survived: %v %v", got, err)
	}
}
...
func TestPolicyRevisionFencesEveryBatchAndCompletion(t *testing.T) {
	...
	for _, v := range publicationFixtures(now, "partial", 400) {
		if err := batch.Set(writeCtx, collection.Doc(v.ID), v); err != nil {
			t.Fatal(err)
		}
	}
	if err := b.Policy().Upsert(ctx, p); err != nil {
	...
	if err := batch.Set(writeCtx, collection.Doc(late.ID), late); err != nil {
		t.Fatal(err)
	}
	if err := batch.Commit(writeCtx); !errors.Is(err, errProjectionInputsChanged) {
		t.Fatalf("late batch accepted: %v", err)
	}
	if err := b.finishProjectionWrite(writeCtx, "alice", true); !errors.Is(err, errProjectionInputsChanged) {
		t.Fatalf("stale completion accepted: %v", err)
	}
	requirePublicationHidden(t, b, ctx)
	b.abandonProjectionWrite(writeCtx, "alice")
	if err := b.Projection().Replace(ctx, "alice", now, now.Add(time.Hour), publicationFixtures(now, "repaired", 1)); err != nil {
		t.Fatal(err)
	}
	got, err := b.Projection().ListForUser(ctx, "alice")
	if err != nil || len(got) != 1 || got[0].ID != "repaired-0000" {
		t.Fatalf("incomplete old-policy rows survived repair: %d %v", len(got), err)
	}
}
...
func TestOverrideRevisionIsAtomicAndIsolated(t *testing.T) {
	b, ctx := emulatorBackend(t)
	now := time.Now().UTC().Truncate(time.Second)
	if err := b.Projection().Replace(ctx, "alice", now, now.Add(24*time.Hour), publicationFixtures(now, "legacy", 1)); err != nil {
		t.Fatal(err)
	}
	...
	if err := b.Policy().CreateOverride(ctx, override); err != nil {
		t.Fatal(err)
	}
	requirePublicationHidden(t, b, ctx)
	if err := b.Projection().Replace(captured, "alice", now, now.Add(24*time.Hour), nil); !errors.Is(err, errProjectionInputsChanged) {
		t.Fatalf("override did not fence old input: %v", err)
	}
	...
	if err != nil || before != after {
		t.Fatalf("failed create advanced revision: %v", err)
	}
	bob := revisionPolicy(now)
	bob.UserID = "bob"
	if err := b.Policy().Upsert(ctx, bob); err != nil {
		t.Fatal(err)
	}
	after, err = decodePolicyRevision(b.policyRevisionRef("alice").Get(ctx))
	if err != nil || before != after {
		t.Fatal("other user's policy changed revision")
	}
}
...
func TestRealRebuilderRejectsConcurrentPolicyChange(t *testing.T) {
	...
	go func() {
		result <- projection.NewRebuilder(paused, b.Policy()).Rebuild(ctx, "alice", now, now.Add(time.Hour), now)
	}()
	select {
	case <-paused.entered:
	case <-ctx.Done():
	...
	if !errors.Is(rebuildErr, errProjectionInputsChanged) {
		t.Fatalf("real rebuild published old input: %v", rebuildErr)
	}
	requirePublicationHidden(t, b, ctx)
	if err := projection.NewRebuilder(b.Calendar(), b.Policy()).Rebuild(ctx, "alice", now, now.Add(time.Hour), now); err != nil {
		t.Fatal(err)
	}
	got, err := b.Projection().ListForUser(ctx, "alice")
	...
	for _, v := range got {
		if v.State.Requestability != policy.RequestClosed {
			t.Fatal("fresh rebuild used old policy")
		}
	}
}
