...
type scopedCancellationStub struct {
	cancellationStore
	organization string
	scopedCalls  int
}

func (s *scopedCancellationStub) CancelConfirmedInOrganization(_ context.Context, id, actor, org, option string) error {
	s.scopedCalls++
	s.organization, s.actor, s.option = org, actor, option
	return s.failure
}

func TestConfirmedCancellationRequiresScopedAuthorization(t *testing.T) {
	for _, tc := range []struct {
		...
		{"authorized replay", "org", coord.ErrAlreadyCancelled, 200, 1},
	} {
		t.Run(tc.name, func(t *testing.T) {
			store := &scopedCancellationStub{}
			...
			req := httptest.NewRequest("POST", "/api/v1/requests/r/cancel-confirmed", strings.NewReader(`{"optionId":"chosen"}`))
			...
			if tc.calls > 0 && (store.organization != tc.org || store.actor != "bob" || store.option != "chosen") {
				t.Fatal("lost authenticated scope/selection")
			}
		})
	}
}

func TestConfirmedCancellationRefusesLegacyStore(t *testing.T) {
