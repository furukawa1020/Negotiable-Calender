...
func TestPostgresCalendarSyncFencing(t *testing.T) {
	...
	for _, scenario := range []string{"reconnect", "disconnect", "expired"} {
		t.Run(scenario, func(t *testing.T) {
			...
			owner, err := store.AcquireSync(ctx, "alice", time.Now().UTC(), time.Minute)
			if err != nil {
				t.Fatal(err)
			}
			if _, err := store.AcquireSync(ctx, "alice", time.Now().UTC(), time.Minute); !errors.Is(err, calendarintegration.ErrSyncBusy) {
			...
			case "disconnect":
				if err := store.DeleteConnection(ctx, "alice"); err != nil {
					t.Fatal(err)
				}
			case "expired":
				exec("UPDATE calendar_connections SET sync_lease_until=$1 WHERE user_id='alice'", now.Add(-time.Minute))
				next, err := store.AcquireSync(ctx, "alice", time.Now().UTC(), time.Minute)
				if err != nil || next.SyncLeaseID == owner.SyncLeaseID {
					t.Fatalf("lease not replaced: %v", err)
				}
			}
			...
			for name, write := range map[string]func() error{
				"events": func() error {
					return store.ApplyChanges(stale, "alice", calendarintegration.ChangeSet{Full: true, Upserts: []calendarintegration.BusySpan{span}}, now, now.Add(time.Hour), now)
				},
				"legacy-events": func() error {
					return store.ReplaceBusySpans(stale, "alice", []calendarintegration.BusySpan{span}, now, now.Add(time.Hour), now)
				},
				"projection": func() error {
					return projection.NewPostgresStore(db).Replace(stale, "alice", now, now.Add(time.Hour), nil)
				},
				...
				if err := write(); !errors.Is(err, calendarintegration.ErrSyncLeaseLost) {
					t.Fatalf("%s stale write: %v", name, err)
				}
			}
			for _, table := range []string{"private_events", "schedule_projections"} {
				...
				if err := db.QueryRowContext(ctx, "SELECT COUNT(*) FROM "+table).Scan(&n); err != nil || n != 0 {
					t.Fatalf("%s stale data: %d %v", table, n, err)
				}
			}
			current, err := store.GetConnection(ctx, "alice")
			if scenario == "disconnect" {
				if !errors.Is(err, calendarintegration.ErrNotFound) {
					t.Fatalf("connection resurrected: %v", err)
				}
			} else if err != nil || current.SyncToken != "" || current.FailureCount != 0 || current.ReconnectRequired {
				t.Fatalf("new connection changed: %+v %v", current, err)
			}
		})
	}
	if err := store.SaveConnection(ctx, connection); err != nil {
	...
	active, err := store.AcquireSync(ctx, "alice", time.Now().UTC(), time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	activeCtx := calendarintegration.WithSyncLease(ctx, calendarintegration.SyncLease{UserID: "alice", ID: active.SyncLeaseID})
	if err := store.MarkSyncSuccess(activeCtx, "alice", "valid", now, now.Add(time.Hour)); err != nil {
		t.Fatal(err)
	}
	if _, err := store.AcquireSync(ctx, "alice", time.Now().UTC(), time.Minute); err != nil {
		t.Fatalf("successful sync did not release lease: %v", err)
	}
}
