...
func GuardSyncTransaction(ctx context.Context, tx *sql.Tx, userID string) error {
	if err := LockCalendarTransaction(ctx, tx, userID); err != nil {
		return err
	}
...
func (store *PostgresStore) AcquireSync(ctx context.Context, userID string, now time.Time, duration time.Duration) (Connection, error) {
	...
	if value.SyncLeaseID != "" && value.SyncLeaseUntil != nil && value.SyncLeaseUntil.After(now) {
		return Connection{}, ErrSyncBusy
	}
	id, until := NewSyncLeaseID(), now.Add(duration)
	if _, err := tx.ExecContext(ctx, "UPDATE calendar_connections SET sync_lease_id=$2,sync_lease_until=$3,last_attempt_at=$4,next_attempt_at=$3 WHERE user_id=$1", userID, id, until, now); err != nil {
		return Connection{}, err
	}
