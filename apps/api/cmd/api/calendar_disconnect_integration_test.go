...
func TestCalendarDisconnectPostgresAtomicity(t *testing.T) {
	...
	if databaseURL=="" { t.Skip("TEST_DATABASE_URL is not configured") }
	...
	exec:=func(query string,args ...any) {
		t.Helper()
		if _,err:=db.ExecContext(ctx,query,args...);err!=nil { t.Fatal(err) }
	}
	...
	for _,id:=range []string{"alice","bob"} {
		...
		if err:=store.SaveConnection(ctx,calendarintegration.Connection{UserID:id,RefreshTokenCipher:[]byte("synthetic"),GrantedScopes:[]string{calendarintegration.CalendarReadonlyScope},ConnectedAt:now});err!=nil { t.Fatal(err) }
		if err:=store.ReplaceBusySpans(ctx,id,[]calendarintegration.BusySpan{{ProviderEventID:"event",CalendarID:"primary",StartAt:now,EndAt:now.Add(time.Hour),Busy:true}},now,now.Add(time.Hour),now);err!=nil { t.Fatal(err) }
	...
	disconnect:=func(want int) {
		...
		if w.Code!=want { t.Fatalf("disconnect status=%d want=%d",w.Code,want) }
	}
	count:=func(table,user string,want int) {
		t.Helper()
		var got int
		if err:=db.QueryRowContext(ctx,"SELECT COUNT(*) FROM "+table+" WHERE user_id=$1",user).Scan(&got);err!=nil { t.Fatal(err) }
		if got!=want { t.Fatalf("%s/%s=%d want=%d",table,user,got,want) }
	}
