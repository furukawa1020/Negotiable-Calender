...
func (*pausedCalendarProvider) Exchange(context.Context,string,string)(calendarintegration.TokenSet,error){return calendarintegration.TokenSet{},nil}
func (p *pausedCalendarProvider) Refresh(ctx context.Context,_ string)(calendarintegration.TokenSet,error){
	close(p.entered)
	select {case <-p.resume:case <-ctx.Done():return calendarintegration.TokenSet{},ctx.Err()}
	if p.fail{return calendarintegration.TokenSet{},calendarintegration.ErrReconnectRequired}
...
func TestCalendarSyncFencingWithPausedProvider(t *testing.T){
	for _,scenario:=range []string{"disconnect","reconnect","stale-failure","expired-lease","overlapping-sync"}{
		t.Run(scenario,func(t *testing.T){
			...
			if err:=b.Calendar().SaveConnection(ctx,connection);err!=nil{t.Fatal(err)}
			...
			select{case <-provider.entered:case <-ctx.Done():t.Fatal(ctx.Err())}
			...
			case "reconnect","stale-failure":
				...
				if err:=b.Calendar().SaveConnection(ctx,connection);err!=nil{t.Fatal(err)}
			case "expired-lease":
				doc,err:=ref.Get(ctx);if err!=nil{t.Fatal(err)}
				var current calendarintegration.Connection
				if err:=doc.DataTo(&current);err!=nil{t.Fatal(err)}
				expired:=now.Add(-time.Minute);current.SyncLeaseUntil=&expired
				putDocument(t,ctx,ref,current)
				if _,err:=b.Calendar().AcquireSync(ctx,"alice",time.Now().UTC(),time.Minute);err!=nil{t.Fatal(err)}
			case "overlapping-sync":
				_,err:=handler.SyncUser(ctx,"alice")
				if !errors.Is(err,calendarintegration.ErrSyncBusy){t.Fatalf("overlap not rejected: %v",err)}
			}
			var before map[string]any
			if scenario!="disconnect"{doc,err:=ref.Get(ctx);if err!=nil{t.Fatal(err)};before=doc.Data()}
			close(provider.resume)
			select{case err=<-done:case <-ctx.Done():t.Fatal(ctx.Err())}
			if scenario=="overlapping-sync"{
				if err!=nil{t.Fatal(err)}
				current,err:=b.Calendar().GetConnection(ctx,"alice")
				if err!=nil||current.SyncToken!="old-cursor"||current.SyncLeaseID!=""{t.Fatalf("valid owner did not finish: %v",err)}
				return
			}
			if err==nil{t.Fatal("obsolete sync succeeded")}
			if scenario=="disconnect"{
				if _,err:=ref.Get(ctx);!firestoreNotFound(err){t.Fatalf("connection resurrected: %v",err)}
			}else{
				doc,err:=ref.Get(ctx);if err!=nil{t.Fatal(err)}
				if !reflect.DeepEqual(before,doc.Data()){t.Fatal("obsolete sync changed new connection or lease")}
			}
			for _,name:=range []string{"privateEvents","scheduleProjections"}{
				...
				if err!=nil||len(docs)!=0{t.Fatalf("obsolete sync wrote %s: %d %v",name,len(docs),err)}
			}
		})
	}
}
...
func TestCalendarFencesEveryWriteAfterReconnect(t *testing.T){
	...
	if err:=b.Calendar().SaveConnection(ctx,connection);err!=nil{t.Fatal(err)}
	lease,err:=b.Calendar().AcquireSync(ctx,"alice",now,time.Minute);if err!=nil{t.Fatal(err)}
	...
	for name,write:=range map[string]func()error{
		"events":func()error{return b.Calendar().ApplyChanges(stale,"alice",calendarintegration.ChangeSet{Upserts:[]calendarintegration.BusySpan{span}},now,now.Add(time.Hour),now)},
		...
		if err:=write();!errors.Is(err,calendarintegration.ErrSyncLeaseLost){t.Fatalf("%s allowed stale writer: %v",name,err)}
	}
	// A queued second batch cannot commit after reconnect, even after the first committed.
	current,err:=b.Calendar().AcquireSync(ctx,"alice",time.Now().UTC(),time.Minute);if err!=nil{t.Fatal(err)}
	...
	if err:=writer.Set(active,ref,map[string]any{"synthetic":true});err!=nil{t.Fatal(err)}
	if err:=writer.Commit(active);err!=nil{t.Fatal(err)}
	if err:=writer.Delete(active,ref);err!=nil{t.Fatal(err)}
	if err:=b.Calendar().SaveConnection(ctx,connection);err!=nil{t.Fatal(err)}
	if err:=writer.Commit(active);!errors.Is(err,calendarintegration.ErrSyncLeaseLost){t.Fatalf("stale batch committed: %v",err)}
	if _,err:=ref.Get(ctx);err!=nil{t.Fatal("stale batch removed published data")}
}
...
func TestCalendarCleanupFencingAndRetry(t *testing.T){
	...
	if err:=b.Calendar().SaveConnection(ctx,calendarintegration.Connection{UserID:"alice"});!errors.Is(err,calendarintegration.ErrSyncBusy){t.Fatalf("reconnect during cleanup: %v",err)}
	if err:=b.Calendar().DeleteConnection(ctx,"alice");!errors.Is(err,calendarintegration.ErrSyncBusy){t.Fatalf("overlapping cleanup: %v",err)}
	...
	if err:=deleteCollection(old,b.Client,b.Client.Collection("users").Doc("alice").Collection("privateEvents"),200);!errors.Is(err,calendarintegration.ErrSyncLeaseLost){t.Fatalf("old cleanup resumed: %v",err)}
}
