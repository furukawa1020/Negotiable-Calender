...
func TestCalendarOAuthSyncPublicationAndDisconnect(t *testing.T) {
	...
	client := &http.Client{Transport: calendarRoundTrip(func(r *http.Request) (*http.Response, error) {
		...
		case "www.googleapis.com/calendar/v3/calendars/primary/events":
			...
			if strings.Contains(r.URL.Query().Get("fields"), "summary") {
				return nil, errors.New("sync requested private event titles")
			}
	...
	connect := func() {
		...
		if w = call(http.MethodGet, callback, cookies...); w.Code != http.StatusFound {
			t.Fatalf("callback: %d %s", w.Code, w.Body.String())
		}
		if w = call(http.MethodGet, callback, cookies...); w.Code != http.StatusBadRequest {
			t.Fatal("OAuth flow reused")
		}
	}
	sync := func(want int) {
		t.Helper()
	...
	for _, d := range docs {
		...
		if strings.Contains(string(raw), "MUST_NOT_PERSIST") || strings.Contains(string(raw), "PRIVATE_DESCRIPTION") {
			t.Fatal("private details persisted by sync")
		}
	}
	...
	assertHidden()
	if _, err := b.Calendar().GetConnection(ctx, "alice"); !errors.Is(err, calendarintegration.ErrNotFound) {
		t.Fatalf("connection remains: %v", err)
	}
	events, err = b.Calendar().ListPrivateEvents(ctx, "alice", now, now.Add(24*time.Hour))
	if err != nil || len(events) != 0 {
		t.Fatal("private events remain")
	}
	if _, err := b.Client.Collection("users").Doc("bob").Collection("scheduleProjections").Doc("other").Get(ctx); err != nil {
		t.Fatal("other user changed")
	}
	// Failed completion must not clear the marker when the connection is gone.
	if err := b.Calendar().MarkSyncSuccess(ctx, "alice", "stale", now, now.Add(time.Hour)); err == nil {
		t.Fatal("missing connection completed")
	}
	assertHidden()
	connect()
	assertHidden()
	...
	assertHidden()
	if _, err := b.Client.Collection("sharingPolicies").Doc("alice").Delete(ctx); err != nil {
		t.Fatal(err)
	}
	sync(http.StatusOK)
	published, err = b.Projection().ListForUser(ctx, "alice")
	if err != nil || len(published) == 0 {
		t.Fatalf("successful resync did not reopen publication: %v", err)
	}
}
...
func TestDisconnectedProjectionMarkerHidesInterruptedCleanup(t *testing.T) {
	...
	if err != nil || values == nil || len(values) != 0 {
		t.Fatalf("interrupted cleanup exposed old data: %v", err)
	}
