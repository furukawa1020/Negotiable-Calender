...
func TestValidateAuthConfig(t *testing.T) {
 ...
 for _, tt := range tests {
  t.Run(tt.name, func(t *testing.T) {
   ...
   if err == nil { t.Fatal("invalid configuration accepted") }
   if !strings.Contains(err.Error(), tt.key) { t.Fatalf("error does not identify setting: %v", err) }
   if strings.Contains(err.Error(), "secret-do-not-log") { t.Fatal("configuration value leaked") }
  })
 }
}
...
func TestValidAuthConfig(t *testing.T) {
 for _, mode := range []string{"false", ""} {
  ...
  if err := validateAuthConfig(func(key string) string { return config[key] }); err != nil { t.Fatal(err) }
 }
}

func TestExplicitDemoNeedsNoOAuthCredentials(t *testing.T) {
 err := validateAuthConfig(func(key string) string { if key == "DEMO_MODE" { return "true" }; return "" })
