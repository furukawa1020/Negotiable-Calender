...
func (b *Backend) beginProjectionWrite(ctx context.Context, userID string, from, to time.Time, deleting bool) (context.Context, error) {
 ...
 err := b.fencedWrite(ctx, userID, func(tx *firestore.Transaction) error {
  ...
  if err == nil {
   if err := doc.DataTo(&previous); err != nil { return err }
  } else if !firestoreNotFound(err) { return err }
  now := time.Now().UTC()
  // Deletion revokes an in-flight replacement before removing any rows.
  if !deleting {
   if previous.LeaseUntil != nil && previous.LeaseUntil.After(now) { return calendarintegration.ErrSyncBusy }
   if previous.Dirty && (previous.ClearRequired || from.After(previous.From) || to.Before(previous.To)) { return errProjectionRepairRequired }
  }
...
func (b *Backend) guardProjectionWrite(ctx context.Context, tx *firestore.Transaction, userID string) error {
 ...
 if value.ID != lease.ID || value.LeaseUntil == nil || !value.LeaseUntil.After(time.Now().UTC()) { return calendarintegration.ErrSyncLeaseLost }
 return nil
}
...
func (b *Backend) projectionReadRevision(ctx context.Context, userID string) (string, bool, error) {
 if _, err := b.projectionBlock(userID).Get(ctx); err == nil { return "", false, nil
 } else if !firestoreNotFound(err) { return "", false, err }
 doc, err := b.projectionPublicationRef(userID).Get(ctx)
 if firestoreNotFound(err) { return "", true, nil }
