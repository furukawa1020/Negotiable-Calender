...
The preview response/prompt is bounded (12 candidates and a 31-day candidate span). Issue #131 replaces post-read length checks with server-limited Firestore queries and account-keyed admission control. Overflow discards all results; there is no fallback to historical full-list reads. Three preview calls occur per successful comparison. See [bounded planning reads](bounded-planning.md) for exact limits, unsupported backends, process-local rate-limit semantics, and billing caveats.
